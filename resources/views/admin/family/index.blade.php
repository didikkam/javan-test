            ...
            <div class="card">
                <div class="card-header">
                    ...
                    <div class="card-tools">
                        <a href="{{ route('admin.family.tree') }}" class="btn btn-info"><i class="fas fa-tree"></i>
                            Tree</a>
                        <a href="{{ route('admin.family.create') }}" class="btn btn-primary"><i class="fas fa-plus"></i>
                            Tambah</a>
                    </div>
                </div>
                <div class="card-body">
    ...
    <script>
        ...
        $(document).ready(function() {
            tabel = $('#tb_data').DataTable({
                ...
                serverSide: true,
                ajax: {
                    url: "{{ route('admin.family.datatable') }}",
                    type: "GET",
                },
                columns: [{
                        data: 'id',
                    ...
                    },
                    {
                        ...
                        render: function(data, type, row, meta) {
                            ...
                            btn +=
                                ...
                                row.id + `/granddaughter" title="Cucu Perempuan"><i class="fas fa-search"></i> Cucu Perempuan</a>`;
                            btn +=
                                `<a class="ml-1 btn btn-info btn-sm" href="{{ route('admin.family.index') }}/` +
                                row.id + `/grandson" title="Cucu"><i class="fas fa-search"></i> Cucu</a>`;
                            btn +=
                                `<a class="ml-1 btn btn-info btn-sm" href="{{ route('admin.family.index') }}/` +
                                row.id + `/child" title="Anak"><i class="fas fa-search"></i> Anak</a>`;
                            btn +=
                                `<a class="ml-1 btn btn-warning btn-sm" href="{{ route('admin.family.index') }}/` +
                                row.id + `/edit" title="Edit"><i class="fas fa-edit"></i> Edit</a>`;
                            btn += `
                                 <form method="POST" name="delete-item" action="{{ route('admin.family.index') }}/` + row
